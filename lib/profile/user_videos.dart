...
class UserVideos extends StatefulWidget {
  ...
  UserVideosState createState() => UserVideosState();
}

class UserVideosState extends State<UserVideos> {
  ...
  void getallvideopost(bool isClear, int currentPage) async {
    ...
    request.other_uid=MemoryManagement.getuserId();
    request.page = currentPage.toString();
    request.limit = limit.toString();

    var response = await _allVideosViewModel.getallvideos(request, context);
